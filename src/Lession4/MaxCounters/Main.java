...
public class Main {
	...
	public static void main(String[] args) {
		int[] list = { 3, 4, 4, 6, 1, 4, 4, 3, 4, 4, 6, 1, 4, 4, 6 };
		// int[] list ={4, 4, 4, 4, 4, 4, 4};
		int[] result = solution(5, list);
		System.out.print("[");
		for (int i : result) {
			System.out.print(i + " ");
		}
	...
	public static int[] counter(int N, int[] counters) {
		counters[N - 1]++;
		return counters;
	}
	// public static int[] maxCount(int[] counters){
	...
	public static int[] solution(int N, int[] A) {
		...
		for (int i = 0; i < A.length; i++) {
			System.out.println("i:"+i+" mV: "+maxValue+
					" maxCount:"+maxCount+
					" A[i]: "+A[i]);
			System.out.print("[");
//			for (int p : counters) {
...
//			
			...
			if (A[i] > 0 && A[i] <= N) {
				if(counters[A[i]-1]== maxValue) maxValue++;
				counters = counter(A[i], counters);
			}
			if (A[i] == N + 1) {
				maxCount += maxValue;
				counters = new int[N];
